...
white='\033[37;1m'
clear
echo -e $red "[!]" $white "This program session 1 running only for termux pkg"
...
pip install npm
pkg install npm && apt-get update && pkg install lua && pkg install luarocks && pip install requests && pip install lolcat && pip install gem
pip install pyfiglet
echo $red"[!]"$white "adb cannot be installed!"
pkg install android-tools
#for linux & superuser command / universal
echo -e $green "[!]" $green "Downloading universal package... ( apt program & su )"
apt update
apt upgrade
sudo apt install toilet
sudo apt install figlet
sudo apt-get update
sudo apt-get fully-update
sudo apt install git
sudo apt install npm && sudo apt install python3 && sudo apt install python3-pip && pip install requests && apt install adb
echo $red "[!]"$white "If your linux doesnt support sudo and shows an error in dpkg, we are gonna fix that ( required superuser ), remember, superuser required an password"
su -c sudo pip install toilet
su -c sudo apt install figlet && su -c sudo pip install pyfiglet
su -c sudo apt-get update
su -c sudo apt install npm && su -c sudo apt install python3 && su -c sudo apt install python3-pip && su -c sudo apt install git && su -c sudo pip install requests && su -c sudo apt install adb
adb start-server
adb kill-server
apt-get update
apt upgrade
...
pip install PyNaCl
pip install kivy
echo $red "[!]" $white "If util-linux package not found, that mean your terminal program doesnt support & have that package."
clear
#!/bin/bash
...
echo -e $green'[*]'$white 'Running...'
clear
toilet -F border -f mono12  "Required"  | lolcat
echo -e $cyan 'Made By HenzX-@'$white
echo -e $white''
echo '[1] proot-distro'
sleep 2
echo '[2] Required package for su'
sleep 2
echo '[3] Python3.12 for root'
sleep 2
echo '[4] userfinder'
sleep 2
echo '[5] adbsploit'
...
echo '[6] grabcam'
sleep 2
echo '[7] superuser reset password'
echo '-----------------------------'
read -p '# > install ' r
if [ $r = 1 ]
then
echo ''
echo -e $red'[!]'$white 'Not supported for root user.'
sleep 2
...
then
clear
toilet -F border -f mono12  "Python3.12"  | lolcat
echo $red'[!]'$white ' BEFORE YOU INSTALL PYTHON3.8 PLEASE INSTALL THE REQUIRED PACKAGE IN THE MENU.'
sleep 5
...
printf "\n"
printf "\e[1;93m             .:.:;..\e[0m\e[1;77m UserFinder v1.0 Developer: menu.sh \e[0m\e[1;93m..;:.:.\e[0m\n"
printf "\n"
...
echo -e $green'[*]'$white 'Type adbsploit to run it.'
exit
fi
if [ $r = 6 ]
then
echo ''
echo 'Do not use this for an illegaly activity.'
pkg install python2
...
if [ $r = 7 ]
then
echo "Since i cannot access your linux account, follow this instruction : "
passwd --help
echo "su"
echo "sudo passwd -e ACCOUNT_TERMINAL"
echo "sudo passwd -u ACCOUNT_TERMINAL"
echo "sudo passwd -d ACCOUNT_TERMINAL"
echo "sudo passwd"
echo
echo "Replace ACCOUNT_TERMINAL with your linux account name, example : "
echo "linux@localhost, so i write the ACCOUNT_TERMINAL with linux, like this, sudo passwd linux"
fi
exit
